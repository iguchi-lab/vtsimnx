...
2. ~~metrics で outer 分布と `aircon_capacity_recalc` 割合を出せるようにする~~（済: mean/max/ge3_share/capacity_share。実ケースでの判定は次）
3. 実ランの `metrics` を見て効果が大きければ thermal に能力固定モード（`Qac=±Qmax`、室温未知）を追加し二分探索を廃止
