...
- `vtsimnx_solver_cpp_test_aircon_controller`
  ...
  - 具体例:
    ...
    - 還気と吹出が同じ風量・同じ循環向きになること（`in == out` でも吹出を逆符号にしない）
    - 設定維持中は `|required_heat_w|` 基準。負荷 0 なら風量 0、定格以上なら `V_dsgn`、`Q_max` で頭打ち
    - 室負荷が無い設定固定と、能力制限中・未達は `Q_max` 基準で、再呼び出しで 0 へ縮小しないこと
