...
**ループの入れ子・図解の正本**は [`simulation_loops.md`](simulation_loops.md) です（外側 / 内側 / 空調の3段階を mermaid で整理）。
...
```mermaid
flowchart TB
    subgraph outer["外側ループ（空調が安定するまで）"]
        inner["内側連成<br/>圧力・熱・（湿気）"]
        aircon["空調評価<br/>風量 / ON-OFF / 能力 / 吹出湿度"]
        inner --> aircon
        aircon -->|再計算| inner
    end
    props["時変プロパティ"] --> outer
    outer -->|Accept| conc["濃度 c"]
    conc --> out["結果出力"]
```

詳細図は [`simulation_loops.md`](simulation_loops.md)。

#### 3.1 反復の外枠

- **圧力＋熱（＋条件により湿気）を内側連成で収束**
- **（湿度内側連成OFF時）湿度 x を 1 ステップ更新**
- **エアコン制御（必要なら外側ループを再計算）**
- **（エアコン制御完了後）濃度 c を 1 ステップ更新**
- **結果出力（artifact）**

補足（連成範囲の考え方）:

- 湿度内側連成が OFF のとき、湿度 \(x\)・濃度 \(c\) は **圧力/熱の収束判定には入りません**（まずは「圧力＋熱」を収束させ、その確定流量で x を更新し、最後に c を更新する方針）。
- 湿度内側連成が ON（既定）のときは、内側反復の収束判定に \(x\)（および潜熱フィードバック有効時は潜熱）も含まれます。詳細は [`simulation_loops.md`](simulation_loops.md) / [`moisture_network_phase1.md`](moisture_network_phase1.md)。
...
4. **圧力-熱（＋湿気）連成の収束判定**
   - 有効な状態量の変化が許容誤差以下になるまで内側反復
5. **湿度計算（`x`）**
   - 内側連成 ON なら内側で更新済み。OFF なら外側で換気流量と `humidity_generation` を使って 1 回更新
6. **エアコン制御**
   - 設定温度/モードに基づきON/OFF等を更新し、必要なら熱計算側を再計算
   - 湿度計算が有効な場合、エアコンモデルへの入力湿度（`X_in/X_ex`）はノードの `current_x` を優先し、取得できない場合は警告してJIS条件へフォールバック
   - 潜熱計算方式は `ac_spec.latent_method` で選択（既定 `rh95`、`bf` 指定時は RH>100% で `rh95` にフォールバック）
   - 能力上限判定は顕熱のみではなく、**全熱（顕熱+潜熱）**で実施
