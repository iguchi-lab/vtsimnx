...
`aircon.pre_rh`（または空調ノードの `pre_rh`）に設定相対湿度 [%] を与えると、**冷房運転中**かつ吸込ノードの絶対湿度が目標を超える場合に、コイルモデルより優先して理想除湿します。

- 目標絶対湿度: `x_sp = absolute_humidity(T_in, pre_rh)`
- 条件: 正の処理風量かつ `x_in > x_sp` のとき `supplyX = x_sp`（この湿度決定式はコイル能力・機種特性に依存しない）
- `pre_rh` 未指定、または目標以下のときは従来どおり `latent_method` を使用
- 暖房 / OFF では無効（既存のパススルー／除外ロジックのまま）

`T_in` と `x_in` は `in_node` の状態であり、`set_node` や `pre_temp` から目標を算定しません。
室のRHを直接拘束する機能でも、湿度超過だけで機器を起動する機能でもありません。
目標超過時は `latent_method="none"` よりも優先し、目標以下では従来モデルによる追加除湿を許します。
後段の全熱能力判定は通常どおり残るため、「機器全体が能力無制限」という意味ではありません。
この分岐では吹出温度での飽和湿度への制限を行わず、過飽和状態を返す可能性があります。
[入力・適用上の注意](../../docs/aircon_humidity_control.md)も参照してください。
...
吹出側の湿気移流は空調ノードの `current_x`（= supplyX）を**固定境界**として使うため、能力計算と湿気・熱移流で吹出湿度を共有します（閾値以下の境界更新では再求解しない点は下記参照）。
...
また各外側ループの物理求解前に、停止中・非冷房、または `current_x` が非有限値もしくは `1e-4 kg/kg(DA)` 以下の空調は吸込湿度へ同期します（`current_x=0` 固定境界で室内が乾燥するのを防ぐ）。
停止中または `current_mode != "COOLING"` の空調ノードに直接接続する換気枝は、吸込側・吹出側とも湿度移流から除外します（換気・熱計算の風量は維持）。冷房ONでは両側を含めます。同一室の循環では水分移動が相殺しますが、異なる室を結ぶ場合は室間水分移動も省く近似となります。文字列比較のため `AUTO` も除外対象です。AUTOの冷房相当運転と湿気輸送の整合は別途確認が必要です。
...
風量の載せ方は枝の種類で分かれる。基準熱量から求める比 `λ = V_target / V_inner.dsgn` は共通である。温度のノードはどちらも同じで、空調機ノードは供給空気、部屋は混合室温である。考え方の正本は [`aircon_control_principles.md`](aircon_control_principles.md) の §5.3。

**固定風量（既定）**

`vol` のみ、または省略。還気（`in_node` → 空調）と吹出（空調 → `out`）の `fixed_flow` に同じ `V_target` を書く。向きはどちらも枝の正方向（吸込は空調へ入り、吹出は空調から出る）。`in == out` のループでも吹出を逆符号にしない。吸込が既に目標でも、吹出がずれていれば更新する。

**ファン PQ**

`p_max`・`p1`・`q1`・`q_max` が揃っているとき。builder は次を作る。

- `in` → 空調: `type=fan`, `subtype=aircon`。定格 PQ を `λ` で縮める（`q' = λ q`、`p' = λ² p`）
- 空調 → `out`: `pressure_loss`（`area` / `k_total`。省略時は `0.05` m² と `1`）

実風量は圧力計算との交点であり、目標 `V_target` を枝へは書かない。`vol` が一緒にあってもファン枝には書かない。ファン枝があるときは `fixed_flow` の更新はしない。圧力計算が必要である。各室への分け前は、利用者が書く `pressure_loss` に任せる。

いずれかが変わった場合は `shouldRecompute=true` を返す。処理熱に使う風量は、どちらの方式でも吸込側である。

注意:

- この補正は DUCT_CENTRAL 専用です（他モデルには適用しません）。
- 同じ枝に固定風量とファン PQ は載せない。
- 停止中はどちらも補正しない。OFF のあと直前の循環が残ることがある。
