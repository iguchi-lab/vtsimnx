...
- 潜熱反映は各内側反復で「基準熱源 + 今回反映分」を再構成し、反復回数依存の積み上がりを抑制
...
- `simulation.coupling.humidity_solver_max_iter`:
  - 湿気内部ソルバ（陰解法）の最大反復回数（既定 `80`）
- `simulation.coupling.humidity_solver_tolerance`:
  - 湿気内部ソルバ（陰解法）の収束許容誤差（既定 `1e-9`）
...
- `transport/humidity_solver.*` は互換ラッパとして維持（内部で core を呼ぶ）
