...
- **時系列の指定**: 多くの値は「定数」または「時系列（配列）」で指定できます。
  ...
  - 時系列対応フィールドでは、配列長は **`simulation.index.length` または 1** です。長さ 1 は全時刻に展開され、その他の長さ（空配列を含む）はエラーになります。型や対象フィールドの詳細は [builder入力仕様 §5](../engine/docs/builder_json.md) を参照してください。
