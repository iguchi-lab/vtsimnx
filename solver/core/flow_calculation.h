...
T calcSimpleOpeningFlow(const T& dp, const EdgeProperties& edgeData) {
    ...
    const T eps   = T(PhysicalConstants::TOLERANCE_SMALL);
    const T abs_dp = (dp >= T(0.0)) ? dp : -dp;
    const T K = alpha * area * sqrt(T(2.0) / T(PhysicalConstants::RHO_AIR));
    if (abs_dp >= eps) {
...
T calcFanFlow(const T& dp, const EdgeProperties& edgeData) {
    ...
    T dp_fan = -dp;
    T tolerance = T(PhysicalConstants::TOLERANCE_MEDIUM);
