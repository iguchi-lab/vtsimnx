...
class WarningDetail(TypedDict, total=False):
    """
    warnings を機械可読にするための構造化情報。
    - code: 警告種別
    - message: 人間向けメッセージ（warnings[str] と同じ文字列）
    - context: セクション/対象（例: "ノード A"）
    - field: フィールド名（例: "unknown_field"）
    - section: 対象セクション（例: "ventilation_branches"）
    - original_key/new_key: key のリネーム前後（重複対処など）
    """
    code: str
    message: str
    context: str
...
def log_and_strip_unknown_fields(data: Dict[str, Any], allowed: Set[str], context: str) -> None:
    """
    互換用: 旧API（warning文字列は返さない）。
    """
    strip_unknown_fields(data, allowed, context)
...
def validate_sim_config(sim_config: Dict[str, Any]) -> Tuple[SimConfigType, ValidationResult]:
    ...
    # 未知キーはログに出しつつ削除
    warnings.extend(strip_unknown_fields(sim_config, _allowed_keys(SimConfigType), "sim_config"))
    ...
    warnings.extend(strip_unknown_fields(sim_config["index"], _allowed_keys(IndexType), "indexセクション"))
    warnings.extend(strip_unknown_fields(sim_config["tolerance"], _allowed_keys(ToleranceType), "toleranceセクション"))
    warnings.extend(strip_unknown_fields(sim_config["calc_flag"], _allowed_keys(CalcFlagType), "calc_flagセクション"))
    ...
    return sim_config, ValidationResult(True, [], warnings)
    ...
    *,
    ...
    for branch in thermal_config:
        ...
        warnings.extend(
            strip_unknown_fields(
                branch,
                _allowed_keys(ThermalBranchType),
                f"熱ブランチ {branch.get('key', '?')}",
                warning_details=warning_details,
            )
        )
    ...
    for branch in branches:
        if "key" in branch:
            ...
            if key_count.get(original_key, 0) > 1:
                ...
                branch["key"] = new_key
                msg = f"{warning_prefix}の'key' '{original_key}' が重複していたため、'{new_key}' に変更しました。"
                warnings.append(msg)
                if warning_details is not None:
                    warning_details.append(
                        {
                            "code": "duplicate_key_renamed",
                            "message": msg,
                            "section": str(warning_prefix),
...
def _check_and_rename_duplicate_keys(
    ...
    *,
    ...
    warnings.extend(_rename_duplicate_keys_in_place(ventilation_config, "換気ブランチ", warning_details=warning_details))

    # 熱ブランチのkey重複をチェックし、重複があれば(01)、(02)を付けてリネーム
    warnings.extend(_rename_duplicate_keys_in_place(thermal_config, "熱ブランチ", warning_details=warning_details))
...
def _validate_sections(config: Dict[str, Any], *, continue_on_error: bool = True) -> Dict[str, Any]:
    ...
    output_json, _warnings = _validate_sections_with_warnings(config, continue_on_error=continue_on_error)
    return output_json


def _validate_sections_with_warnings(config: Dict[str, Any], *, continue_on_error: bool = True) -> Tuple[Dict[str, Any], List[str]]:
    """ファイル I/O を伴わない共通バリデーション処理。正規化済み JSON 互換 dict と warnings を返す。"""
    ...
    all_warnings.extend(sim_result.warnings)
    ...
    if not vent_result.is_valid:
        ...
        if not continue_on_error:
            raise ValidationError("\n".join(all_errors))
    all_warnings.extend(vent_result.warnings)
    ...
    if not thermal_result.is_valid:
        ...
        if not continue_on_error:
            raise ValidationError("\n".join(all_errors))
    all_warnings.extend(thermal_result.warnings)
    ...
    return convert_to_json_compatible(output_json), all_warnings


def _validate_sections_with_warning_details(
    config: Dict[str, Any], *, continue_on_error: bool = True
) -> Tuple[Dict[str, Any], List[str], List[WarningDetail]]:
    """
    ファイル I/O を伴わない共通バリデーション処理。
    正規化済み JSON 互換 dict と warnings（文字列/構造化）を返す。
    """
    sim_config = config["simulation"]
    node_config = config["nodes"]
    ventilation_config = config["ventilation_branches"]
    ...
    all_errors: List[str] = []
    all_warnings: List[str] = []
    all_warning_details: List[WarningDetail] = []
    ...
    node_config, node_result = validate_node_config(
        sim_config, node_config, ventilation_config, thermal_config, warning_details=all_warning_details
    )
    if not node_result.is_valid:
        all_errors.extend(node_result.errors)
        if not continue_on_error:
    ...
    ventilation_config, vent_result = validate_ventilation_config(
        sim_config, node_config, ventilation_config, warning_details=all_warning_details
    )
    if not vent_result.is_valid:
        all_errors.extend(vent_result.errors)
        if not continue_on_error:
    ...
    thermal_config, thermal_result = validate_thermal_config(
        sim_config, node_config, thermal_config, warning_details=all_warning_details
    )
    if not thermal_result.is_valid:
        all_errors.extend(thermal_result.errors)
        if not continue_on_error:
    ...
    duplicate_errors, duplicate_warnings = _check_and_rename_duplicate_keys(
        node_config, ventilation_config, thermal_config, warning_details=all_warning_details
    )
    all_errors.extend(duplicate_errors)
    for warning in duplicate_warnings:
        logger.warning(warning)
    ...
    return convert_to_json_compatible(output_json), all_warnings, all_warning_details
...
def validate_with_warnings(config_path: str, *, continue_on_error: bool = True) -> Tuple[Dict[str, Any], List[str]]:
    """
    指定した JSON 設定のバリデーションを行い、正規化後の JSON と warnings を返す。
    成功時は temp ディレクトリに JSON を保存する。
    """
    logger.info("validation(with_warnings)を開始します。")

    raw_config = validate_config_file(config_path)
    output_json, warnings = _validate_sections_with_warnings(raw_config, continue_on_error=continue_on_error)

    # temp へ保存（validate と同様）
    input_stem = Path(config_path).stem.replace("_02", "")
    output_dir = Path(__file__).resolve().parent.parent / "temp"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / f"{input_stem}.json"
    ...
    logger.info("validation(with_warnings)が正常に終了しました。")
    return output_json, warnings
...
def validate_dict_with_warnings(config: Dict[str, Any], *, continue_on_error: bool = True) -> Tuple[Dict[str, Any], List[str]]:
    """
    既にロード済みの設定 dict を検証・正規化し、JSON 互換化した dict と warnings を返す。
    ファイル I/O は行わない。
    """
    for section in ["simulation", "nodes", "ventilation_branches", "thermal_branches"]:
        if section not in config:
            raise ConfigFileError(f"設定データに'{section}'セクションがありません")
    return _validate_sections_with_warnings(config, continue_on_error=continue_on_error)


def validate_dict_with_warning_details(
    config: Dict[str, Any], *, continue_on_error: bool = True
) -> Tuple[Dict[str, Any], List[str], List[WarningDetail]]:
    """
    既にロード済みの設定 dict を検証・正規化し、JSON 互換化した dict と warnings（文字列/構造化）を返す。
    ファイル I/O は行わない。
    """
    for section in ["simulation", "nodes", "ventilation_branches", "thermal_branches"]:
        if section not in config:
            raise ConfigFileError(f"設定データに'{section}'セクションがありません")
    return _validate_sections_with_warning_details(config, continue_on_error=continue_on_error)
